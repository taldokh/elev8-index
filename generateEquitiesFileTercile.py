...
# Function to select stocks using spaced/tercile selection
def select_tercile_stocks(stocks_df, equities_per_firm):
    selected_stocks = []
...
def generate_index_holdings(output_file_path=cg.RESULT_EQUITIES_FILE_PATH):
    ...
    for quarter_directory in quarter_directories:
        ...
        selected_stocks = select_tercile_stocks(stocks_df, EQUITIES_PER_FIRM)
...
def generate_equities_file_tercile(equities_per_firm, number_of_firms):
    ...
    print(f'Selecting {equities_per_firm} stocks per {number_of_firms} with the TERCILE selection method')
